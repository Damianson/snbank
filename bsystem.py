...
def options():
    firstChoice = input(
        "Input only the corresponding number to select an option\n\t1 Staff Login\n\t2 Close App\n\t> ")
    if firstChoice == '1':
        ...
        def check_password(username, password, lines):
            for line in lines:
                if username in line and password in line:
                    ...
                    def optionFour():
                        login = input("Input only the corresponding number to select an option\n\t1 Create new bank account\n\t2 Check Account details\n\t3 Logout\n\t> ")
                        if login == '1':
                            ...
                            def balance():
                                ...
                                if openingBalance.isdigit():
                                    ...
                                    print("Opening balance must be integer only\n")
                                    balance()
                            balance()
